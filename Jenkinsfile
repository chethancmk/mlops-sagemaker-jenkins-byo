pipeline {
    ...
    stages {
        ...
        stage("BuildPushContainer") {
            steps {
              sh """
                echo "${params.ECRURI}"
                aws ecr get-login-password --region '${env.AWS_REGION}' | docker login --username AWS --password-stdin ${params.ECRURI}
            ...
            steps { 
              sh """
               if ! aws cloudformation describe-stacks --region '${env.AWS_REGION}' --stack-name '${env.SAGEMAKER_TRAINING_JOB}'-test ; then
                  ...
                  aws cloudformation create-stack --region '${env.AWS_REGION}' --stack-name '${env.SAGEMAKER_TRAINING_JOB}'-test --template-body file://deploy/cfn-sagemaker-endpoint.yml --parameters ParameterKey=ModelName,ParameterValue=\"${env.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}\" ParameterKey=ModelDataUrl,ParameterValue=\"s3://${S3_MODEL_ARTIFACTS}/${env.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}-${currentBuild.startTimeInMillis}\"/output/model.tar.gz ParameterKey=TrainingImage,ParameterValue="${params.ECRURI}:${env.BUILD_ID}" ParameterKey=InstanceType,ParameterValue='ml.t2.large'  ParameterKey=InstanceCount,ParameterValue='1' ParameterKey=RoleArn,ParameterValue="${params.SAGEMAKER_EXECUTION_ROLE_TEST}" ParameterKey=Environment,ParameterValue='Test'
                  echo "Waiting for stack to be created ..."
                  aws cloudformation wait stack-create-complete --region '${env.AWS_REGION}' --stack-name "${env.SAGEMAKER_TRAINING_JOB}"-test
               else
                  echo -e '\nStack exists, attempting update ...'
                  set +e
                  update_output=`aws cloudformation update-stack --region '${env.AWS_REGION}' --stack-name '${env.SAGEMAKER_TRAINING_JOB}'-test --template-body file://deploy/cfn-sagemaker-endpoint.yml --parameters  ParameterKey=ModelName,ParameterValue=\"${env.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}\" ParameterKey=ModelDataUrl,ParameterValue=\"s3://${S3_MODEL_ARTIFACTS}/${env.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}-${currentBuild.startTimeInMillis}\"/output/model.tar.gz ParameterKey=TrainingImage,ParameterValue="${params.ECRURI}:${env.BUILD_ID}" ParameterKey=InstanceType,ParameterValue='ml.t2.large'  ParameterKey=InstanceCount,ParameterValue='1' ParameterKey=RoleArn,ParameterValue="${params.SAGEMAKER_EXECUTION_ROLE_TEST}" ParameterKey=Environment,ParameterValue='Test'`
                  status=\$?
                  set -e
                  echo \$update_output
                  if [ \$status -ne 0 ] ; then
	...
	stage("TestEvaluate") {
      ...
      stage("DeployToProd") {
            steps { 
              sh """
               if ! aws cloudformation describe-stacks --region '${env.AWS_REGION}' --stack-name '${env.SAGEMAKER_TRAINING_JOB}'-prod ; then
                  echo -e "\nStack does not exist, creating ..."
                  ...
                  echo "Waiting for stack to be created ..."
                  aws cloudformation wait stack-create-complete --region '${env.AWS_REGION}' --stack-name "${env.SAGEMAKER_TRAINING_JOB}"-prod
               else
                  echo -e '\nStack exists, attempting update ...'
                  set +e
                  update_output=`aws cloudformation update-stack --region '${env.AWS_REGION}' --stack-name '${env.SAGEMAKER_TRAINING_JOB}'-prod --template-body file://deploy/cfn-sagemaker-endpoint.yml --parameters  ParameterKey=ModelName,ParameterValue=\"${env.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}\" ParameterKey=ModelDataUrl,ParameterValue=\"s3://${S3_MODEL_ARTIFACTS}/${env.SAGEMAKER_TRAINING_JOB}-${env.BUILD_ID}-${currentBuild.startTimeInMillis}\"/output/model.tar.gz ParameterKey=TrainingImage,ParameterValue="${params.ECRURI}:${env.BUILD_ID}" ParameterKey=InstanceType,ParameterValue='ml.t2.large'  ParameterKey=InstanceCount,ParameterValue='1' ParameterKey=RoleArn,ParameterValue="${params.SAGEMAKER_EXECUTION_ROLE_TEST}" ParameterKey=Environment,ParameterValue='Prod'`
               ...
               echo "Waiting for stack update to complete ..."
               aws cloudformation wait stack-update-complete --region '${env.AWS_REGION}' --stack-name '${env.SAGEMAKER_TRAINING_JOB}'-prod
               fi
               echo "Finished create/update successfully!"
              """
             }
